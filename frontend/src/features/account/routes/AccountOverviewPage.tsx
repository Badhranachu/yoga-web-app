  ...
  return (
    <div className="space-y-8">
      ...
      <UserDashboardPanel title="Upcoming Bookings">
        {upcomingBookings.length === 0 ? <p className="text-sm text-brown">No upcoming bookings.</p> : <div className="divide-y divide-beige">{upcomingBookings.map((booking) => { const effectiveStatus = getEffectiveStatus(booking); return <div key={booking.id} className="flex items-center justify-between gap-4 py-3 text-sm"><div><p className="text-dark">{formatDate(booking.slot.date)}</p><p className="text-xs text-brown">{formatTime(booking.slot.start_time)} – {formatTime(booking.slot.end_time)}</p></div><div className="flex items-center gap-3"><span className={`text-xs uppercase tracking-wider ${EFFECTIVE_STATUS_STYLES[effectiveStatus]}`}>{EFFECTIVE_STATUS_LABEL[effectiveStatus]}</span>{effectiveStatus !== 'expired' && <button onClick={() => setRescheduleBooking(booking)} className="text-xs uppercase tracking-widest text-brown hover:text-gold-dark">Reschedule</button>}</div></div>; })}</div>}
